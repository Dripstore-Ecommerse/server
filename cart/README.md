...
This microservice manages the cart catalog and provides information about the available carts.
...
- **Process management**: advanced cartion process management using [PM2](https://pm2.keymetrics.io)
...
# run docker container in cartion mode
yarn docker:prod
...
In cartion mode, only `info`, `warn`, and `error` logs will be printed to the console.\
It is up to the server (or process manager) to actually read them from the console and store them in log files.\
This app uses pm2 in cartion mode, which is already configured to store the logs in log files.
